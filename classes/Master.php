...
Class Master extends DBConnection {
	...
	function delete_room(){
		...
		if($del){
			$resp['status'] = 'success';
			if(is_dir(base_app.'uploads/room_'.$id)){
				$file = scandir(base_app.'uploads/room_'.$id);
				foreach($file as $img){
					if(in_array($img,array('..','.')))
						continue;
					unlink(base_app.'uploads/room_'.$id.'/'.$img);
				}
				rmdir(base_app.'uploads/room_'.$id);
			}
			$this->settings->set_flashdata('success',"Tour room successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_booking(){
		...
		if($del){
			$resp['status'] = 'success';
			if(is_dir(base_app.'uploads/user_id'.$id)){
				$file = scandir(base_app.'uploads/user_id'.$id);
				foreach($file as $img){
					if(in_array($img,array('..','.')))
						continue;
					unlink(base_app.'uploads/user_id'.$id.'/'.$img);
				}
				rmdir(base_app.'uploads/user_id'.$id);
			}
			$this->settings->set_flashdata('success',"Tour booking successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_inquiry(){
		...
		if($del){
			$resp['status'] = 'success';
			if(is_dir(base_app.'uploads/name_'.$id)){
				$file = scandir(base_app.'uploads/name_'.$id);
				foreach($file as $img){
					if(in_array($img,array('..','.')))
						continue;
					unlink(base_app.'uploads/name_'.$id.'/'.$img);
				}
				rmdir(base_app.'uploads/name_'.$id);
			}
			$this->settings->set_flashdata('success',"Tour inquiry successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
